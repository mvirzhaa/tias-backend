...
// Sebelumnya periode "aktif" di-hardcode ke satu UUID tetap di kode (mis. "2025 Ganjil").
// Begitu SIAK pindah periode akademik (pergantian semester), kelas periode lama hilang dari
// daftar dan kelas periode baru pun tidak ikut muncul (UUID beda) — kelas jadi tampak hilang
// total untuk semua dosen/mahasiswa sampai ada yang sadar & deploy ulang dengan UUID baru.
// SIAK sendiri tidak (belum terverifikasi) menyediakan endpoint daftar periode akademik,
// jadi di sini periode berjalan di-resolve dari data yang sudah tersinkron: periode dengan
// jumlah kelas is_active=true TERBANYAK dianggap periode berjalan — is_active sendiri sudah
// di-refresh SIAK tiap sync lewat statusKelas (lihat syncService.isAktif), jadi ini otomatis
// mengikuti pergantian semester tanpa perlu deploy manual.
async function resolveActivePeriode() {
  const [rows] = await db.query(`
    SELECT "siakPeriodeAkademikId", count(*)::int AS active_count
    FROM siak_v2_classes
    WHERE is_active = true AND "siakPeriodeAkademikId" IS NOT NULL
    GROUP BY "siakPeriodeAkademikId"
    ORDER BY active_count DESC
    LIMIT 1
  `);
  return rows[0] ? rows[0].siakPeriodeAkademikId : null;
}
...
async function listLmsClasses(options = {}) {
  ...
  const periodeAktif = await resolveActivePeriode();
  ...
  // Scope daftar kelas ke periode akademik aktif (semester berjalan) — dilewati kalau belum
  // ada kelas is_active sama sekali (mis. sebelum sync pertama) daripada memaksa WHERE dengan
  // parameter null yang tidak akan pernah match apa pun.
  const where = ['cls.is_active = true'];
  if (periodeAktif) {
    where.push('cls."siakPeriodeAkademikId" = :periodeAktif');
  }
