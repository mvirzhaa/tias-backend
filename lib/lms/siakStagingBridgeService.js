...
async function listLmsClasses(options = {}) {
  ...
  // Kelas SIAK yang is_active=true tapi belum ada dosen ATAUPUN peserta sama sekali —
  // biasanya penawaran periode lain yang belum berjalan/tidak dipakai, muncul sebagai
  // "duplikat" visual dari kelas yang sama nama/kode/label-nya tapi periode beda.
  if (!includeEmpty) {
    where.push(`(
      EXISTS (SELECT 1 FROM siak_v2_class_lecturers l WHERE l."kelasKuliahId" = cls."kelasKuliahId")
      OR EXISTS (SELECT 1 FROM siak_v2_participants p WHERE p."kelasKuliahId" = cls."kelasKuliahId")
    )`);
  }
