...
/**
 * Presensi — payload { metode, durasi_menit? }. State SESI (token, geofence, roster) tetap
 * hidup di tabel lms_attendance_sessions/lms_attendance_records, bukan di sini — tapi
 * `metode`/`durasi_menit` dipilih dosen SEKALI saat membuat aktivitas ini, lalu dipakai
 * otomatis tiap kali sesi baru dibuka utk pertemuan ini (bukan ditebak ulang dari jadwal
 * SIAK tiap kali, dan bukan diminta ulang tiap buka sesi).
 *
 * `metode` WAJIB ('online'|'offline') — dosen yang menentukan eksplisit, sesuai permintaan
 * (sebelumnya cuma auto-detect dari SiakV2Jadwal.metode_pembelajaran, dosen tidak py kendali).
 * `durasi_menit` OPSIONAL, angka > 0, dinamis (dosen bebas isi berapa saja) — sesi otomatis
 * dianggap berakhir sekian menit sejak dibuka; kosong = tanpa batas waktu (tutup manual saja,
 * perilaku lama). Item LAMA (dibuat sebelum validator ini ada, payload `{}`) tidak error saat
 * dibaca — cuma tidak bisa disimpan ulang sampai dosen pilih metode saat edit; openSession
 * tetap fallback ke deteksi jadwal SIAK selama item belum diedit ulang (lihat
 * controllers/lms/attendanceController.js).
 */
function validateAttendance(payload) {
  if (!isPlainObject(payload)) {
    return err("payload Presensi harus object { metode, durasi_menit? }.");
  }
  const { metode, durasi_menit } = payload;

  if (metode !== "online" && metode !== "offline") {
    return err("metode wajib diisi: 'online' atau 'offline'.");
  }
  ...
  if (durasi_menit != null && String(durasi_menit).trim() !== "") {
    ...
    if (!Number.isFinite(n) || n <= 0) {
      return err("durasi_menit harus angka > 0 (atau dikosongkan = tanpa batas waktu).");
    }
    durasi = Math.round(n);
  }

  return ok({ metode, durasi_menit: durasi });
}
