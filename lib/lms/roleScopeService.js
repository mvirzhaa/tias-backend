...
async function resolveAcademicScope({ role_key, scope_type, fakultas_id, prodi_id }) {
  ...
  if (scope_type === "faculty") {
    ...
    // DEGRADASI: siak_v2_program_studi."siakFakultasId" baru terisi setelah master prodi
    // tersedia. Sebelum itu, scope fakultas tak bisa dibuat (tak ada dimensi pencocok).
    const [rows] = await db.query(
      `SELECT DISTINCT "siakFakultasId" FROM siak_v2_program_studi
       WHERE "siakFakultasId" = :fakultasId LIMIT 1`,
      { replacements: { fakultasId: fakultas_id } }
    );
    if (!rows.length) throw new Error("fakultas_id tidak ditemukan di SIAK v2 (dimensi prodi belum terisi).");
  ...
  if (scope_type === "study_program") {
    ...
    const [rows] = await db.query(
      `SELECT "siakProgramStudiId" AS prodi_id, "siakFakultasId" AS fakultas_id
       FROM siak_v2_program_studi WHERE "siakProgramStudiId" = :prodiId LIMIT 1`,
      { replacements: { prodiId: prodi_id } }
    );
    if (!rows.length) throw new Error("prodi_id tidak ditemukan di SIAK v2.");
...
async function listAcademicUnits() {
  const [rows] = await db.query(`
    SELECT
      "siakProgramStudiId" AS prodi_id,
      kode_prodi,
      nama_prodi,
      "siakFakultasId" AS fakultas_id,
      nama_fakultas
    FROM siak_v2_program_studi
    WHERE is_active = true
    ORDER BY nama_fakultas ASC NULLS LAST, nama_prodi ASC
  `);
  return rows;
}
...
async function getClassAcademicScope(kelasKuliahId) {
  ...
  const [rows] = await db.query(`
    SELECT
      cls."kelasKuliahId" AS kelas_kuliah_id,
      cls."siakProgramStudiId" AS prodi_id,
      ps.kode_prodi,
      ps.nama_prodi,
      ps."siakFakultasId" AS fakultas_id
