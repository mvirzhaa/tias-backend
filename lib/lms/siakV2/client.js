...
function createClient() {
  ...
  // SIAK internal kini via HTTPS dengan sertifikat self-signed. Bila
  // SIAK_V2_INSECURE_TLS === 'true', terima cert tak terverifikasi HANYA untuk
  // instance axios SIAK ini (tidak global, tidak via NODE_TLS_REJECT_UNAUTHORIZED).
  // Default: verifikasi TLS tetap aktif.
  const insecure = process.env.SIAK_V2_INSECURE_TLS === "true";
  const httpsAgent = new https.Agent({ rejectUnauthorized: !insecure });

  return axios.create({ baseURL, timeout: TIMEOUT_MS, headers, httpsAgent });
}
