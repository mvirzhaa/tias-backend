 ...
 *
 * NIDN dibandingkan tanpa leading zero (mis. tb_users.nidn "0411058902" vs SIAK
 * "411058902") — ketahuan dari kasus dosen yang match by nama tapi 0 baris by NIDN
 * padahal keduanya "sama". btrim saja tidak cukup karena itu cuma buang whitespace,
 * bukan normalisasi padding angka. NPM sengaja TIDAK ikut dinormalisasi (belum ada
 * bukti masalah serupa di sana, dan leading digit NPM = tahun angkatan — bukan
 * padding yang aman dibuang begitu saja).
 */
async function linkIdentities(rows, { siakField, identifierType, userColumn }) {
  ...
  const stripLeadingZeros = identifierType === "nidn";
  const columnExpr = stripLeadingZeros
    ? `regexp_replace(btrim(${userColumn}), '^0+(?=.)', '')`
    : `btrim(${userColumn})`;
  const identifierExpr = stripLeadingZeros
    ? `regexp_replace(btrim(:identifier), '^0+(?=.)', '')`
    : `btrim(:identifier)`;
  ...
  for (const row of rows) {
    ...
    const [users] = await db.query(
      `SELECT user_id FROM tb_users
       WHERE deleted_at IS NULL AND ${columnExpr} = ${identifierExpr}`,
