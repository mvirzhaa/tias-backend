 ...
 *
 * `scope` ('dosen' | 'mahasiswa' | 'both', default 'both') — dosen: sertakan SARAN
 * match berbasis nama (similarity pg_trgm, fallback ILIKE; JANGAN auto-link, cuma
 * saran). Tidak dipaginasi: jumlah dosen jauh lebih kecil & saran sudah dibatasi
 * top-5/baris. Mahasiswa: sertakan match NPM langsung dari tb_users jika ada.
 * DIPAGINASI + bisa di-search (nama/NPM) — daftar ini bisa mencapai ribuan baris di
 * kampus besar. FE (tab Mahasiswa) sebaiknya panggil scope='mahasiswa' saja — kalau
 * scope='both' dipakai untuk setiap keystroke pencarian/ganti halaman, similarity
 * dosen ikut dihitung ulang tiap kali padahal tak berubah, sia-sia.
 */
async function listUnmatchedUsers({ scope = "both", mahasiswaSearch, mahasiswaPage, mahasiswaLimit } = {}) {
  const wantDosen = scope === "dosen" || scope === "both";
  const wantMahasiswa = scope === "mahasiswa" || scope === "both";
  const mhsLimit = normalizeLimit(mahasiswaLimit);
  const mhsPageNum = normalizePage(mahasiswaPage);
  const mhsOffset = (mhsPageNum - 1) * mhsLimit;
  const mhsSearchTerm = mahasiswaSearch ? String(mahasiswaSearch).trim() : "";

  let dosenWithSuggestions = [];
  let hasTrgm = false;
  ...
  if (wantDosen) {
    ...
    // Nama dosen diambil langsung dari siak_v2_class_lecturers.nama (diisi sync v2
    // dari jadwalKuliah[].dosen.nama). nidn/nama identik per siak_dosen_id → max() aman.
    ...
    // Untuk setiap dosen SIAK yang unmatched, cari kandidat TIAS terdekat secara
    // nama. similarity() jika pg_trgm tersedia, fallback token ILIKE.
    dosenWithSuggestions = unmatchedDosen.map((siakDosen) => {
      let candidates = [];
  ...
  if (wantMahasiswa) {
    // ── 4. Mahasiswa unmatched dari siak_v2_participants (dipaginasi + searchable) ──
    const mhsCte = `
      WITH unmatched AS (
      ...
      WHERE (:mhsSearchTerm = '' OR nama_siak ILIKE :mhsSearchPattern OR npm ILIKE :mhsSearchPattern)
    `;
    const mhsReplacements = {
      mhsSearchTerm,
      mhsSearchPattern: `%${mhsSearchTerm}%`,
      mhsLimit,
      mhsOffset,
    };

    const [mhsCountRows] = await db.query(
      `SELECT count(*)::int AS total FROM (${mhsCte}) t`,
      { replacements: mhsReplacements }
    );
    mhsTotal = mhsCountRows[0].total;

    const [mhsRows] = await db.query(
      `${mhsCte} ORDER BY nama_siak ASC NULLS LAST LIMIT :mhsLimit OFFSET :mhsOffset`,
      { replacements: mhsReplacements }
    );
    unmatchedMhs = mhsRows;
  }
  ...
  return {
    dosen: wantDosen
      ...
      : null,
    mahasiswa: wantMahasiswa
      ? {
          limit: mhsLimit,
          page: mhsPageNum,
          total: mhsTotal,
          total_page: Math.ceil(mhsTotal / mhsLimit),
          rows: unmatchedMhs,
          note: "match_status='npm_match' artinya ada akun TIAS dengan NPM sama. Tetap perlu dikonfirmasi admin.",
        }
      : null,
    similarity_engine: wantDosen ? (hasTrgm ? "pg_trgm" : "token_ilike_fallback") : null,
  };
}
...
/**
 * bulkCreateUserMappings — POST /siak-sync/user-mappings/bulk
 * Konfirmasi banyak mapping sekaligus (mis. semua mahasiswa match_status='npm_match'
 * yang dicentang admin) — tanpa ini, admin harus klik satu-persatu untuk tiap orang,
 * yang di kampus besar bisa ratusan/ribuan klik. Reuses createUserMapping per pair
 * (validasi & conflict-check yang sama) tapi tidak all-or-nothing: satu pair gagal
 * (mis. sudah keburu di-mapping orang lain) tidak menggagalkan pair lain di batch ini.
 */
async function bulkCreateUserMappings(pairs = [], adminUserId) {
  if (!Array.isArray(pairs) || !pairs.length) throw new Error("pairs wajib diisi (array, minimal 1).");
  if (pairs.length > MAX_LIMIT) throw new Error(`Maksimal ${MAX_LIMIT} pair per batch.`);
  ...
  for (const pair of pairs) {
    try {
      const mapping = await createUserMapping(pair, adminUserId);
      created.push(mapping);
    } catch (error) {
      failed.push({ tias_user_id: pair?.tias_user_id, siak_user_uuid: pair?.siak_user_uuid, error: error.message });
    }
  }
