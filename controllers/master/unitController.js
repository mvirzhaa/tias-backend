...
class UnitController {
  ...
  static create = async (req, res) => {
    try {
      const checkName = await Unit.findOne({
        where: {
          code: req.body.code,
      ...
      if (checkName) {
        return response(
          res,
          false,
          "Error, code dan nama unit sudah digunakan!",
          null
        );
      }
  ...
  static update = async (req, res) => {
    try {
      ...
      const check = await Unit.findOne({
        where: {
          id,
        },
      });
      ...
      if (!check) {
        return response(res, false, "Unit Tidak ditemukan ", null);
      }
      ...
      const update = await Unit.update(casting, {
        where: {
          id,
        },
      });
