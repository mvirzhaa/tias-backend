...
class KategoriKegiatanController {
  static index = async (req, res) => {
    try {
      ...
      const data = await KategoriKegiatan.findAndCountAll({
        distinct: true,
        where: condition,
        order: [[order, orderBy]],
  ...
  static all = async (req, res) => {
    try {
      const data = await KategoriKegiatan.findAll();
  ...
  static detail = async (req, res) => {
    try {
      ...
      const data = await KategoriKegiatan.findOne({
        where: {
          id,
        },
      });
  ...
  static create = async (req, res) => {
    try {
      const checkName = await KategoriKegiatan.findOne({
        where: {
      ...
      const save = await KategoriKegiatan.create({
        ...casting,
      });
  ...
  static update = async (req, res) => {
    try {
      ...
      const check = await KategoriKegiatan.findOne({
        where: {
          id,
        },
      });
      ...
      if (!check) {
        return response(res, false, "Kategori Kegiatan Tidak ditemukan ", null);
      }
      ...
      const update = await KategoriKegiatan.update(casting, {
        where: {
          id,
        },
      });
  ...
  static destroy = async (req, res) => {
    try {
      ...
      await KategoriKegiatan.destroy({
        where: { id },
      });
